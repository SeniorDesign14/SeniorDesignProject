...
def get_hallId(h_name):
    df_h = pd.read_csv('/home/pyuser/data/dininghalls.csv')

    # Find the id for given dining hall
    for index,row in df_h.iterrows():
        if row['hallName'] == h_name:
            # print(h_name,row['diningHallID'])
            h_id = row['diningHallID']
            return h_id
...
def parse_menu_data():
    ...
    file = open('/home/pyuser/data/schedule.csv','w',newline='')
    writer = csv.writer(file)
    writer.writerow(['isBreakfast','isLunch','isDinner','food','diningHall','scheduleDate','diningStation','allergens'])
    ...
    for index, row in df.iterrows():
        ...
        # Change date to month-day-year string format
        date_unclean = row['Menu Date'] 
        date_obj = datetime.strptime(date_unclean, '%A, %B %d, %Y')
            ...
            if pd.isna(menu):
                ...
                for index1, item in enumerate(clean_menu):  # Iterate through each item in menu
                    ...
                    if item.startswith('--') and item.endswith('--'):
                        ...
                        # Add new dining stations to csv
                        # Check for edge case, Gelfenbien in csv has comma in naming. cant find id, other name does not have comma
                        if 'Gelfenbien Commons' in dining_hall:
                            h_id = 2
                        else:
                            h_id = get_hallId(dining_hall)
                        df_s = pd.read_csv('/home/pyuser/data/diningstations.csv')

                        # If station does not exist in csv
                        if not ((df_s['stationName'] == curr_sec) & (df_s['diningHallID'] == h_id)).any(): 
             
                            file = open('/home/pyuser/data/diningstations.csv','a',newline='')
                            writer1 = csv.writer(file)
                            writer1.writerow([curr_sec,h_id])
                            print(f"{curr_sec} with diningHallID {h_id} written to csv")
                        # else:
                        #     print(f"{curr_sec} with diningHallID {h_id} already exists")
                        ...
                        if "Allergens" not in item and "Allergens" in clean_menu[index1+1]:
                            ...
                            writer.writerow([breakfast_index,lunch_index,dinner_index,item,dining_hall,date_clean,curr_sec,item_allergens])
                

                        # If food doesnt have allergen
                        if "Allergens" not in item and "Allergens" not in clean_menu[index1+1]:
                            writer.writerow([breakfast_index,lunch_index,dinner_index,item,dining_hall,date_clean,curr_sec])
        
def parse_nutrition_info():
    '''Clean nutrition info'''
    df = pd.read_csv('nutritioninfo.csv')
    file = open('/home/pyuser/data/nutritionalinfo.csv','w',newline='')
    writer = csv.writer(file)
